...
class LSGANTrainer(BaseGANTrainer):
    ...
    def gen_loss(self, gen_imgs):
        disc_out = self.model.discriminator(gen_imgs).requires_grad_(True)
        g_loss = self.criterion(disc_out, torch.full([self.current_batch_size], self.gen_c, dtype=torch.float32).to(self.device))
    ...
    def d_fake_loss(self, gen_imgs):
        ...
        d_fake_loss = self.criterion(d_out_fake, torch.full([self.current_batch_size], self.dis_a, dtype=torch.float32).to(self.device))
    ...
    def d_real_loss(self, real_imgs):
        ...
        d_real_loss = self.criterion(d_out_real, torch.full([self.current_batch_size], self.dis_b, dtype=torch.float32).to(self.device))
